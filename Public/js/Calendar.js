 ...
 * --------------------------------------------------------
 * --------------------------------------------------------
 * @version 1.3 build 2015-04-03
 * @modifier	Yoon (yinchangshuai@gmail.com)
 * @new		    add Hours Minutes Seconds
 * --------------------------------------------------------
 */
function Calendar(beginYear, endYear, language, patternDelimiter, date2StringPattern, string2DatePattern,showTime) {//add showTime
	this.showTime = showTime;//add showTime
...
Calendar.prototype.draw = function() {
	...
	//add showTime st
	_cs[_cs.length] = ' <tr id="trTime"><th colspan="7">';
	_cs[_cs.length] = '  <select id="houseSelect" name="houseSelect"><\/select>:<select id="minSelect" name="minSelect"><\/select>:<select id="secSelect" name="secSelect"><\/select>';
	_cs[_cs.length] = '  <input type="button" class="ok" name="okButton" id="okButton" value="确定" \/><\/th>';
	_cs[_cs.length] = ' <\/tr>';
	//add showTime ed
	_cs[_cs.length] = ' <tr>';
	...
	for(var i=0;i<=23;i++){
		var n=(i<10)?"0"+i:i;
		this.form.houseSelect.options[this.form.houseSelect.length] = new Option(n, i);
	}
	for(var i=0;i<=59;i++){
		var n=(i<10)?"0"+i:i;
		this.form.minSelect.options[this.form.minSelect.length] = new Option(n,i );
		this.form.secSelect.options[this.form.secSelect.length] = new Option(n,i );
	}
	this.form.secSelect.onchange=this.form.houseSelect.onchange=this.form.minSelect.onchange=function(){
	...
	this.form.okButton.onclick=function(){ calendar.dateControl.value = calendar.date.format(calendar.date2StringPattern);calendar.hide();}//add showTime
	...
	this.form.selectTodayButton.onclick = function () {
		...
		calendar.house = today.getHours();//add showTime
		calendar.min = today.getMinutes();//add showTime
		calendar.second=today.getSeconds();//add showTime
...
Calendar.prototype.changeSelect = function() {
	...
	var ms = this.form.monthSelect;
	//add showTime st
	var hs = this.form.houseSelect;
	var ds = this.form.minSelect;
	var ss = this.form.secSelect;
	//add showTime ed
	...
	for (var i= 0; i < ds.length; i++){
		if (ds.options[i].value == this.date.getMinutes()){
			ds[i].selected = true;
			break;
		}
	}
...
Calendar.prototype.updateDate = function (n){
	...
	if(this.showTime){
		var h=this.form.houseSelect;
		this.house=h.options[h.selectedIndex].value;
		h=this.form.minSelect;
		this.min=h.options[h.selectedIndex].value;
		h=this.form.secSelect;
...
Calendar.prototype.bindData = function () {
	...
	if(this.showTime){
		panel.style.height="238px";
	}else{
		panel.style.height="209px";
			...
			tds[i].onclick = function () {
				...
				if (calendar.dateControl){
					calendar.dateControl.value = new Date(calendar.date.getFullYear(),
														...
														this.innerHTML,
														calendar.date.getHours(),//add showTime
														calendar.date.getMinutes(),//add showTime
														calendar.date.getSeconds()//add showTime
...
Calendar.prototype.show = function (dateControl, popuControl,showTime) {//add showTime
	//add showTime st
	...
	this.button = popuControl;//add by Yoon, hide the calendar when click other area expcept the inputbox
	popuControl = dateControl;//add by Yoon, hide the calendar when click other area expcept the inputbox
...
document.writeln('<div id="__calendarPanel" style="position:absolute;visibility:hidden;z-index:9999;background-color:#FFFFFF;border:1px solid #666666;width:200px;height:209px;">');
...
__ci.document.writeln('th {font-size:12px;font-weight:normal;background-color:#FFFFFF;padding: 1px;height:20px}');
...
__ci.document.writeln('td {font-size:12px;text-align:center;height:16px}');
...
__ci.document.writeln('input.ok {border: 1px solid #CCCCCC;background-color:#EFEFEF;width:49px;height:20px;}');
__ci.document.writeln('-->');
 ...
 * add by Yoon, hide the calendar when click other area expcept the inputbox
 */
document.onclick=function(){
	// if(calendar.dateControl != window.event.srcElement) calendar.hide();//add by jiangxinglong
	if((calendar.dateControl != window.event.srcElement)&&(calendar.button != window.event.srcElement)) calendar.hide();
}